            ...
            json.person.map(person => { //插入评论内容
                ...
                if (person.reply.type == 'reply') {
                    ...
                    <div>
                        ...
                        <div id="oli${person.id}" name="reply${person.id}" class="replyBox"></div>
                        <div class="hr">
                            <div>
...
function nowLangLength() {
    var length = lang.value.length;
    setInterval(function() {
        nowLangLength = length;
    }, 1000);
}
nowLangLength();
...
function toOutputLang(str) {
    ...
    var reg_g = /\$\{(.+?)\}/g;
    var length;
    if (str.match(reg_g) == null) {
        length = 0;
    } else {
        length = str.match(reg_g).length;
    }
